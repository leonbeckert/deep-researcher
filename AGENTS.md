...
4. **WebFetch first, browser second.** Try WebFetch for content extraction. Only use browser tools when WebFetch returns empty or the page requires JavaScript rendering.

5. **PDF reading chain.** Download PDFs with `curl -sL -o`. Read with file reading tools (≤100 pages). For larger PDFs, extract text with `pdftotext` and read the .txt output.
...
- Export: After writing the .md report, convert to .pdf and .docx using the commands in `skills/export.md`. If the user explicitly requests only specific formats, follow their request. Otherwise, all three formats are mandatory.
...
This installs pandoc, typst, poppler, tesseract, and Playwright. Works on macOS and Linux. On Windows, it prints manual install instructions.
